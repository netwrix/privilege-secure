...
## Search for resources that are not a part of the given policy id. (Auth policies: MfaRequired; roles: Admin,UserPlus)
...
Invoke-RestMethod -Method GET -Url "$($NPSUrl)/api/v1/AccessControlPolicy/SearchResourceCandidates/{accessPolicyId} -Headers $Headers -ContentType "application/json"
```
...
<h3 id="search-for-resources-that-are-not-a-part-of-the-given-policy-id.-(auth-policies:-mfarequired;-roles:-admin,userplus)-parameters">Parameters</h3>
...
<h3 id="search-for-resources-that-are-not-a-part-of-the-given-policy-id.-(auth-policies:-mfarequired;-roles:-admin,userplus)-responses">Responses</h3>
