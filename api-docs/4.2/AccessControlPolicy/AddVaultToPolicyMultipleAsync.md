...
## Add secret vaults to access policy (Auth policies: MfaRequired; roles: Admin)
...
Invoke-RestMethod -Method PUT -Url "$($NPSUrl)/api/v1/AccessControlPolicy/{policyId}/SecretVault" -ContentType "application/json" -Body $JsonBody -Headers $Headers -ContentType "application/json"
```
...
<h3 id="add-secret-vaults-to-access-policy-(auth-policies:-mfarequired;-roles:-admin)-parameters">Parameters</h3>
...
<h3 id="add-secret-vaults-to-access-policy-(auth-policies:-mfarequired;-roles:-admin)-responses">Responses</h3>
