...
## Remove secret vaults from access policy (Auth policies: MfaRequired; roles: Admin)
...
Invoke-RestMethod -Method DELETE -Url "$($NPSUrl)/api/v1/AccessControlPolicy/{policyId}/SecretVault" -ContentType "application/json" -Body $JsonBody -Headers $Headers -ContentType "application/json"
```
...
<h3 id="remove-secret-vaults-from-access-policy-(auth-policies:-mfarequired;-roles:-admin)-parameters">Parameters</h3>
...
<h3 id="remove-secret-vaults-from-access-policy-(auth-policies:-mfarequired;-roles:-admin)-responses">Responses</h3>
