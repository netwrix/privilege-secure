...
## Get scanned user. (Auth roles: Admin,App)
...
curl -X GET /api/v1/HostScanJob/User/{userId} \
...
Invoke-RestMethod -Method GET -Url "$($NPSUrl)/api/v1/HostScanJob/User/{userId} -Headers $Headers -ContentType "application/json"
```


</details>

`GET /api/v1/HostScanJob/User/{userId}`

<h3 id="get-scanned-user.-(auth-roles:-admin,app)-parameters">Parameters</h3>
...
<h3 id="get-scanned-user.-(auth-roles:-admin,app)-responses">Responses</h3>
