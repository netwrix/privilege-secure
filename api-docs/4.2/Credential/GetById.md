...
## Get specific credential by Id (Auth policies: MfaRequired; roles: Admin,App)
...
Invoke-RestMethod -Method GET -Uri "$($NPSUrl)/api/v1/Credential/{credentialId} -Headers $Headers -ContentType "application/json"
```
...
If showPassword is true, then an event will be sent that a user
has viewed the password. Events are forwarded to SIEM servers if
the SIEM Service has been configured.

<h3 id="get-specific-credential-by-id-(auth-policies:-mfarequired;-roles:-admin,app)-parameters">Parameters</h3>

|Name|In|Type|Required|Description|
|---|---|---|---|---|
|credentialId|path|string(uuid)|true|Credential Id|
|showPassword|query|boolean|false|Include the password with the credential|
...
<h3 id="get-specific-credential-by-id-(auth-policies:-mfarequired;-roles:-admin,app)-responses">Responses</h3>
